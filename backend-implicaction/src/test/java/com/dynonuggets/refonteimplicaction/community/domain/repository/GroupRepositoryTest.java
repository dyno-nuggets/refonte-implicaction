...
class GroupRepositoryTest extends AbstractContainerBaseTest {
    ...
    void findAllByTopPosting() {
        ...
        final List<Group> groups = asList(s1, s2, s3, s4, s5, s6, s7, s8, s9);

        groupRepository.saveAll(groups);

        final List<Post> s1Posts = Stream.of(1, 2, 3, 4, 5)
                .map(id -> Post.builder().name("bla").group(s1).build())
                .collect(toList());

        final List<Post> s2Posts = Stream.of(1, 2, 3, 4, 5, 6, 7)
                .map(id -> Post.builder().name("bla").group(s2).build())
                .collect(toList());

        final List<Post> s3Posts = Stream.of(1, 2)
                .map(id -> Post.builder().name("bla").group(s3).build())
                .collect(toList());

        final List<Post> s5Posts = Stream.of(1, 2, 3)
                .map(id -> Post.builder().name("bla").group(s5).build())
                .collect(toList());

        final List<Post> s6Posts = Stream.of(1, 2, 3, 4, 5)
                .map(id -> Post.builder().name("bla").group(s6).build())
                .collect(toList());

        final List<Post> s8Posts = Stream.of(1, 2, 3, 4, 5, 8, 9)
